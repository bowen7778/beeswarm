...
async function release() {
  ...
  log(`版本演进: ${oldVersion} -> ${newVersion}`);
  ...
  if (process.argv.includes('--push')) {
    try {
      ...
      log(`>>> 自动化发布指令已下发！请前往 https://github.com/bowen7778/beemcp/actions 观察构建进度。`, '32');
    } catch (err) {
      log('Git 推送失败，请手动执行: git push origin main --tags', '31');
    }
  } else {
