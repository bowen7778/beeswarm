...
@injectable()
export class CodexCliChannelDriver implements ChannelDriver {
  readonly channelType: ChannelType = "cli_codex";

  supports(mode: ProjectModeType, channel: ChannelType): boolean {
    return mode === "single_agent" && channel === "cli_codex";
  }
  ...
  async handleCommand(command: CommandEnvelope): Promise<ChannelCommandResult> {
    return {
      ...
      error: {
        ...
        message: "Codex CLI channel is reserved and not implemented yet",
  ...
  async getStatus(projectId: string, mode: ProjectModeType): Promise<ChannelRuntimeStatus> {
    return {
      ...
      lastErrorMessage: "Codex CLI channel is reserved and not implemented yet",
      lastHeartbeatAt: "",
      version: "cli-codex-driver-1"
    };
  }
}
